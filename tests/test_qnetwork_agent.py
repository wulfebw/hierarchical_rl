...
class TestQNetwork(unittest.TestCase):
    ...
    def test_qnetwork_training(self):
        ...
        num_actions = len(mdp.get_actions(None))
        discount = mdp.get_discount()
        a = qnetwork_agent.QNetworkAgent(num_actions=num_actions, input_shape=input_shape, discount=discount, exploration_prob=.5, step_size=1e-3, frozen_update_period=1000, replay_memory_capacity=10000, batch_size=100, exploration_prob_reduction_steps=100000, min_exploration_prob=0.05, logging=True)
        num_epochs = 10
        epoch_length = 1000
        test_epoch_length = 0
        max_steps = 100
